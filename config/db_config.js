...
var launch = function (connection, callback) {
    ...
    connection.query("CREATE TABLE IF NOT EXISTS `hypertube`.`tv_shows` ( `id` INT(5) NOT NULL AUTO_INCREMENT , `title` VARCHAR(255) NOT NULL DEFAULT 'N/A' , `season` VARCHAR(5) NOT NULL DEFAULT 'N/A' , `genres` VARCHAR(255) NOT NULL DEFAULT 'N/A' , `director` TEXT NOT NULL, `writers` TEXT NOT NULL, `actors` TEXT NOT NULL, `summary` TEXT NOT NULL , `cover` VARCHAR(255) NOT NULL DEFAULT 'N/A' , `imdb_code` VARCHAR(255) NOT NULL DEFAULT 'N/A' , `rating` DECIMAL(2,1) DEFAULT NULL , `year` VARCHAR(255) NOT NULL DEFAULT 'N/A', `eztv_id` INT(5) NOT NULL , PRIMARY KEY (`id`)) ENGINE = InnoDB;");
                                            ...
                                            if (torrents) {
                                                torrents.episodes.forEach(function (torrent) {
                                                    if (torrent.quality === "480p") {
                                                        ...
                                                        connection.query("INSERT INTO `hypertube`.`tv_shows_torrents`(id_tv_show, season,episode,magnet,quality) VALUES(?,?,?,?,?)", [rows.insertId, torrent.seasonNumber, torrent.episodeNumber, url, torrent.quality], function (err) {
                                                            if (err) {
                                                                console.log("----------------------------");
                                                                console.log(torrent)
                                                            }
