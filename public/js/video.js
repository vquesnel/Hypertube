...
['msRequestFullscreen', 'msExitFullscreen', 'msFullscreenElement', 'msFullscreenEnabled', 'MSFullscreenChange', 'MSFullscreenError']];
		...
		51: [function (_dereq_, module, exports) {
			...
			var Player = function (_Component) {
				...
				function Player(tag, options, ready) {
					...
					if (options.languages) {
						(function () { // Normalise player option languages to lowercase
				...
				Player.prototype.createEl = function createEl() {
					...
					Object.getOwnPropertyNames(attrs).forEach(function (attr) { // workaround so we don't totally break IE7
				...
				Player.prototype.dimension = function dimension(_dimension, value) {
					...
					if (value === '') { // If an empty string is given, reset the dimension to be automatic
				...
				Player.prototype.updateStyleEl_ = function updateStyleEl_() {
					...
					if (this.width_ !== undefined) { // Use any width that's been specifically set
						width = this.width_;
					} else if (this.height_ !== undefined) { // Or calulate the width from the aspect ratio if a height has been set
				...
				Player.prototype.handleTechReady_ = function handleTechReady_() {
					...
					if ((this.src() || this.currentSrc()) && this.tag && this.options_.autoplay && this.paused()) {
						try { // Chrome Fix. Fixed in Chrome v16.
				...
				Player.prototype.handleTechLoadStart_ = function handleTechLoadStart_() { // TODO: Update to use `emptied` event instead. See #1277.
					...
					} else { // reset the hasStarted state
				...
				Player.prototype.hasStarted = function hasStarted(_hasStarted) {
					if (_hasStarted !== undefined) { // only update if this is a new value
				...
				Player.prototype.techGet_ = function techGet_(method) {
					if (this.tech_ && this.tech_.isReady_) { // Flash likes to die and reload when you hide or reposition it.
						// In these cases the object methods go away and we get errors.
						// When that happens we'll catch the errors and inform tech that it's not ready any more.
						try {
							return this.tech_[method]();
						} catch (e) { // When building additional tech libs, an expected method may not be defined yet
				...
				Player.prototype.duration = function duration(seconds) {
					...
					if (seconds !== this.cache_.duration) { // Cache the last set value for optimized scrubbing (esp. Flash)
				...
				Player.prototype.volume = function volume(percentAsDecimal) {
					var vol = void 0;
					if (percentAsDecimal !== undefined) { // Force value to between 0 and 1
				...
				Player.prototype.requestFullscreen = function requestFullscreen() {
					...
					if (fsApi.requestFullscreen) { // the browser supports going fullscreen at the element level so we can
					...
					} else if (this.tech_.supportsFullScreen()) { // we can't take the video.js controls fullscreen but we can go fullscreen
				...
				Player.prototype.sourceList_ = function sourceList_(sources) {
					...
					if (sourceTech) {
						if (sourceTech.tech === this.techName_) { // if this technology is already loaded, set the source
							this.src(sourceTech.source);
						} else { // load this technology with the chosen source
							this.loadTech_(sourceTech.tech, sourceTech.source);
						}
						this.cache_.sources = sources;
					} else { // We need to wrap this in a timeout to give folks a chance to add error event handlers
						this.setTimeout(function () {
							this.error({
								code: 4,
				...
				Player.prototype.userActive = function userActive(bool) {
					if (bool !== undefined) {
						...
						if (bool !== this.userActive_) {
							...
							if (bool) { // If the user was inactive and is now active we want to reset the
								// inactivity timer
								this.userActivity_ = true;
							...
							} else { // We're switching the state to inactive manually, so erase any other
								// activity
				...
				Player.prototype.listenForUserActivity_ = function listenForUserActivity_() {
					...
					var handleMouseMove = function handleMouseMove(e) { // #1068 - Prevent mousemove spamming
					...
					this.setInterval(function () { // Check to see if mouse/touch activity has happened
						if (this.userActivity_) { // Reset the activity tracker
							this.userActivity_ = false; // If the user state was inactive, set the state to active
							this.userActive(true); // Clear any existing inactivity timeout to start the timer over
							this.clearTimeout(inactivityTimeout);
							var timeout = this.options_.inactivityTimeout;
							if (timeout > 0) { // In <timeout> milliseconds, if no more activity has occurred the
			...
			Player.prototype.options_ = { // Default order of fallback technology
				...
				language: navigator && (navigator.languages && navigator.languages[0] || navigator.userLanguage || navigator.language) || 'en', // locales and their language translations
